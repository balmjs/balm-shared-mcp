...
describe('Performance Integration Tests', () => {
        ...
        async queryComponent(name, category) {
          try {
            const componentsData = JSON.parse(
              await fs.readFile(
                path.join(__dirname, '../fixtures/yiban-shared/components.json'),
                'utf8'
              )
            );
        ...
        async getBestPractices(topic) {
          try {
            const practicesData = JSON.parse(
              await fs.readFile(
                path.join(__dirname, '../fixtures/yiban-shared/best-practices.json'),
                'utf8'
              )
            );
  ...
  describe('Component Query Performance', () => {
    it('should handle multiple concurrent component queries efficiently', async () => {
      ...
      try {
        const results = await Promise.all(queries.map(query => mcpServer.queryComponent(query)));
      ...
      try {
        ...
        results.forEach((result, index) => {
          ...
          if (result.status === 'fulfilled') {
            ...
            expect(result.value).toContain(`invalid-component-${index}`);
          }
        });
      } catch (error) {
      ...
      try {
        ...
        results.forEach((result, index) => {
          if (result.status === 'fulfilled') {
            ...
            expect(result.value).toContain(validTopics[index]);
          } else {
